...
class Piece {
  ...
  constructor(id: String) {
    ...
    if (arr.size == 2) {
      ...
      this.position = Position(if (this.type == PieceType.QUEEN) 3 else 4, if (this.color == Color.WHITE) 0 else 7)
    } else if (arr.size == 3) {
  ...
  private fun getStartPosition(type: PieceType, color: Color, columnChar: Char): Position {
    ...
    var column = if (columnChar in '0'..'9') columnChar.toInt() - 1 else throw IllegalArgumentException("Invalid column character: $columnChar")
...
data class Position(val column: Int, val row: Int, var piece: Piece? = null) {
  init {
